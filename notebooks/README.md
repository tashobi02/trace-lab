...
Restart the kernel and run all cells in a workflow. Workflows locate the root marker and explicitly execute the definitions notebook via IPython `%run`; functions/classes/imports are defined there and a loading confirmation is displayed. Every code cell retains an actual execution count and visible output. Saved outputs describe the validation machine and its configured assets; re-execute to inspect another machine. No model scoring, Unity execution or training is implemented yet. Those capabilities will be added with their own verification.
