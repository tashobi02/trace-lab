...
Run the workspace-inspection notebook first, then controller validation and runtime verification. Always restart the kernel and run all cells. Environment verification checks packages/imports/CPU arithmetic; it does not imply model, simulator, reward or training verification.

## Execute without an editor

Use the controller interpreter from the clone root. This example saves an executed copy, including a failing cell if one occurs, without overwriting the source notebook:
